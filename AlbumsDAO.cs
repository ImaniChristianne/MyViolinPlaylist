    ...
    internal class AlbumsDAO
    {
        ...
        public List<Album> searchTitles(String searchTerm)
        {
            ...
            command.CommandText = "SELECT ID, ALBUM_TITLE, ARTIST, YEAR, " +
                "IMAGE_NAME, DESCRIPTION FROM ALBUMS WHERE " +
        ...
        internal int addOneAlbum(Album album)
        {
            ...
            MySqlCommand command = new MySqlCommand("INSERT INTO `albums`(`ALBUM_TITLE`, " +
                ...
                "VALUES (@albumtitle, @artist, @year, @imageURL," +
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    Track t = new Track
                    {
                        ...
                        purchase_description = reader.GetString(4),
                    };
        ...
        public List<JObject> getTracksUsingJoin(int albumID)
        {
            ...
            command.CommandText = "SELECT tracks.ID as trackID, albums.ALBUM_TITLE, " +
                "`track_title`, `video_url`, `purchase_description" +
                "`FROM `tracks` JOIN albums ON albums_ID = albums.ID WHERE albums_ID = @albumid";
            command.Parameters.AddWithValue("@albumid", albumID);
            command.Connection = connection;
        ...
        internal int deleteTrack(int trackID)
        {
            ...
            MySqlCommand command = new MySqlCommand("DELETE FROM `tracks` WHERE" +
                "`tracks`.`ID` = @trackID; ", connection);
